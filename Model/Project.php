...
class Project{
    ...
    public function getProjects($cid){
        ...
        public function getProjectSs($cid){
        $this->db->query("SELECT *  FROM club_projects WHERE cid=:cid ORDER BY post_date DESC");
        $this->db->bind(":cid",$cid);
        $row = $this->db->resultSet();
        return $row;
    }
    ...
    public function getTodaysProjects(){
        $this->db->query("SELECT club_projects.* ,iclub.*, iclub.name as club_name ,club_projects.name as proj_name FROM `club_projects` JOIN iclub on club_projects.cid=iclub.cid ORDER BY from_date ASC");
        $row=$this->db->resultSet();
        return $row;
...
//     }
    ...
    public function getProjectW($cid){
        $this->db->query("SELECT * FROM club_projects WHERE  cid=:cid ORDER BY from_date DESC");
        $this->db->bind(":cid",$cid);
        $rows=$this->db->resultSet();
        $newRow=[];
        foreach($rows as $row){           
